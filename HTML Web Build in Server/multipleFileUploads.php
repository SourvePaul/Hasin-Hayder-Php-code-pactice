...
$allowedTypes = array ( 
 ...
 if($_FILES['pic']) {
    ...
    $totalFiles = count($_FILES['pic']['name']);
    for($i=0; $i<$totalFiles; $i++) {
        if( in_array($_FILES['pic']['type'][$i], $allowedTypes) !== false && $_FILES['pic']['size'][$i] < 5*1024*1024) {
          echo move_uploaded_file($_FILES['pic']['tmp_name'][$i],"imageFiles/".$_FILES['pic']['name'][$i]);
        }
    }
 }
